...
const RT_CONE = 5;    // a triangle with 3 vertices.
const RT_GLASS   = 6;    // Implicit surface:Blinn-style Gaussian 'blobbies'.
...
// ellipsoids for the unit sphere and rectangles (or prisms) from the unit box.
	...
	switch(this.shapeType) {
	  case RT_GNDPLANE: //--------------------------------------------------------
    	...
    	this.lineColorA = vec4.fromValues(0.0,0.2,0.0,1.0);  // RGBA green(A==opacity)
    	this.gapColorA = vec4.fromValues( 0.3,0.3,0.3,1.0);  // near-white
    	this.lineColorD = vec4.fromValues(0.1,0.5,0.1,1.0);  // RGBA green(A==opacity)
    	this.gapColorD = vec4.fromValues( 0.5,0.5,0.5,1.0);  // near-white
      this.shiness = 30;
	    break;
	  case RT_DISK: //------------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
    	...
    	this.gapColorA = vec4.fromValues( 0.3,0.3,0.3,1.0);  // near-white
    	this.lineColorD = vec4.fromValues(0.1,0.5,0.1,1.0);  // RGBA green(A==opacity)
    	this.gapColorD = vec4.fromValues( 0.9,0.9,0.9,1.0);  // near-white
      this.shiness = 30;
	    break;
	  case RT_SPHERE: //----------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceSphere(inR,hit); }; 
    	this.lineColorA = vec4.fromValues(0.0,0.1,0.3,1.0);  // RGBA blue(A==opacity)
    	this.lineColorD = vec4.fromValues(0.0,0.3,1.0,1.0);  // RGBA blue(A==opacity)
      this.shiness = 20;
	    break;
	  case RT_BOX:    //----------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceBox(inR,hit);    }; 
    	this.lineColorA = vec4.fromValues(0.0,0.5,0.3,1.0);  // RGBA blue(A==opacity)
    	this.lineColorD = vec4.fromValues(0.0,0.7,1.0,1.0);  // RGBA blue(A==opacity)
      this.shiness = 70;
	    break;
	  case RT_CYLINDER://---------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceCyl(inR,hit);    }; 
    	this.lineColorA = vec4.fromValues(0.5,0.3,0.0,1.0);  // RGBA blue(A==opacity)
    	this.lineColorD = vec4.fromValues(0.7,1.0,0.0,1.0);  // RGBA blue(A==opacity)
      this.shiness = 50;
	    break;
	  case RT_CONE://---------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceCone(inR,hit);    }; 
    	this.lineColorA = vec4.fromValues(0.3,0.0,0.5,1.0);  // RGBA blue(A==opacity)
    	this.lineColorD = vec4.fromValues(1.0,0.0,0.7,1.0);  // RGBA blue(A==opacity)
      this.shiness = 20;
	    break;
	  case RT_GLASS://-----------------------------------------------------------
	    //set the ray-tracing function (so we call it using item[i].traceMe() )
	    this.traceMe = function(inR,hit) { this.traceSphere(inR,hit); }; 
      this.traceTrans = function(inR,hit) { this.traceGlass(inR,hit); };
    	this.lineColorA = vec4.fromValues(0.0,0.1,0.3,1.0);  // RGBA blue(A==opacity)
    	this.lineColorD = vec4.fromValues(0.0,0.3,1.0,1.0);  // RGBA blue(A==opacity)
      this.shiness = 80;
	    break;
...
*/
  ...
  myHit.hitList.add(this, myHit.hitPt);                        
...
//------------------ Step 2: Test 1st triangle. Did ray MISS sphere entirely?
  ...
  if(L2 <= 1.00001) { // report error and quit.  LATER we can use this case to
  ...
  if(t0hit <= 0 || t0hit > myHit.t0) {    // is this new hit-point CLOSER than 'myHit'?
  ...
  myHit.surfNorm = vec4.fromValues(myHit.surfNorm[0], myHit.surfNorm[1], myHit.surfNorm[2] ,0)
  vec4.normalize(myHit.surfNorm, myHit.surfNorm);
...
CGeom.prototype.traceBox = function(inRay, myHit){
  ...
  var ts = new Array();
  ts.push((1-rayT.orig[0])/rayT.dir[0]);
  ts.push((-1-rayT.orig[0])/rayT.dir[0]);
  ts.push((1-rayT.orig[1])/rayT.dir[1]);
  ts.push((-1-rayT.orig[1])/rayT.dir[1]);
  ts.push((1-rayT.orig[2])/rayT.dir[2]);
  ts.push((-1-rayT.orig[2])/rayT.dir[2]);
  ...
  for(var i = 0; i < ts.length; i++){
    switch(i){
      case 0:
      if(ts[i] > 0 && ts[i] < myHit.t0){
        if((rayT.orig[1]+rayT.dir[1]*ts[i] <= 1 && rayT.orig[1]+rayT.dir[1]*ts[i] >= -1)&&(rayT.orig[2]+rayT.dir[2]*ts[i] <= 1 && rayT.orig[2]+rayT.dir[2]*ts[i] >= -1)){
          hit = i;
          myHit.t0 = ts[i];
          nor = vec4.fromValues(1,0,0,0);
        }
      }
      break;
      case 1:
      ...
      case 2:
      if(ts[i] > 0 && ts[i] < myHit.t0){
        if((rayT.orig[0]+rayT.dir[0]*ts[i] <= 1 && rayT.orig[0]+rayT.dir[0]*ts[i] >= -1)&&(rayT.orig[2]+rayT.dir[2]*ts[i] <= 1 && rayT.orig[2]+rayT.dir[2]*ts[i] >= -1)){
          hit = i;
          myHit.t0 = ts[i];
          nor = vec4.fromValues(0,1,0,0);
        }
      }
      break;
      case 3:
      ...
      case 4:
      if(ts[i] > 0 && ts[i] < myHit.t0){
        if((rayT.orig[0]+rayT.dir[0]*ts[i] <= 1 && rayT.orig[0]+rayT.dir[0]*ts[i] >= -1)&&(rayT.orig[1]+rayT.dir[1]*ts[i] <= 1 && rayT.orig[1]+rayT.dir[1]*ts[i] >= -1)){
          hit = i;
          myHit.t0 = ts[i];
          nor = vec4.fromValues(0,0,1,0);
        }
      }
      break;
      case 5:
  ...
  // in model space we know it's always +z,
  // but we need to TRANSFORM the normal to world-space, & re-normalize it.
  vec4.transformMat4(myHit.surfNorm, nor, this.normal2world);
  myHit.surfNorm = vec4.fromValues(myHit.surfNorm[0], myHit.surfNorm[1], myHit.surfNorm[2] ,0)
  vec4.normalize(myHit.surfNorm, myHit.surfNorm);
  myHit.hitNum = 1;   // in CScene.makeRayTracedImage, use 'this.gapColor'
  myHit.hitList.add(this, myHit.hitPt);                        
...
CGeom.prototype.traceCyl = function(inRay, myHit){
  ...
  vec4.transformMat4(rayT.orig, inRay.orig, this.worldRay2model);
  vec4.transformMat4(rayT.dir,  inRay.dir,  this.worldRay2model);
  var a = vec3.fromValues(0-rayT.orig[0], 0-rayT.orig[1],0);
  var b = vec3.fromValues(rayT.dir[0],rayT.dir[1],0);
  var dot = vec3.dot(a,b);
  var mora = Math.sqrt(Math.pow(rayT.orig[0],2)+Math.pow(rayT.orig[1],2));
  var morb = Math.sqrt(Math.pow(rayT.dir[0],2)+Math.pow(rayT.dir[1],2));
  var cos = dot/mora/morb;
  if(cos < 0){
    return;
  }
  var dis = Math.sqrt(Math.pow(mora,2) - Math.pow(mora * cos,2));
  if(dis > 1){
    return;
  }
  var A = Math.pow(rayT.dir[0],2) + Math.pow(rayT.dir[1],2);
  var B = 2 * (rayT.orig[0] * rayT.dir[0] + rayT.orig[1] * rayT.dir[1])
  var C = Math.pow(rayT.orig[0],2) + Math.pow(rayT.orig[1],2) - 1;
  ...
  // Compute the x,y,z,w point where inRay hit the grid-plane in WORLD coords:
    vec4.scaleAndAdd(myHit.hitPt, inRay.orig, inRay.dir, myHit.t0);
    nor = vec4.fromValues(myHit.modelHitPt[0],myHit.modelHitPt[1],0,0);
  }
  else{
    var tag = 0;
    newt1 = (1-rayT.orig[2])/rayT.dir[2];
    newt2 = (-1-rayT.orig[2])/rayT.dir[2];
    dis1 = Math.sqrt(Math.pow(rayT.orig[0] + rayT.dir[0] * newt1, 2) + Math.pow(rayT.orig[1] + rayT.dir[1] * newt1, 2));
    dis2 = Math.sqrt(Math.pow(rayT.orig[0] + rayT.dir[0] * newt2, 2) + Math.pow(rayT.orig[1] + rayT.dir[1] * newt2, 2));
    if(newt1 > 0 && newt1 < myHit.t0 && dis1 < 1){
      myHit.t0 = newt1;
      vec4.scaleAndAdd(myHit.modelHitPt, rayT.orig, rayT.dir, myHit.t0); 
  // Compute the x,y,z,w point where inRay hit the grid-plane in WORLD coords:
      vec4.scaleAndAdd(myHit.hitPt, inRay.orig, inRay.dir, myHit.t0);
    ...
    if(newt2 > 0 && newt2 < myHit.t0 && dis2 < 1){
      myHit.t0 = newt2;
      vec4.scaleAndAdd(myHit.modelHitPt, rayT.orig, rayT.dir, myHit.t0); 
  // Compute the x,y,z,w point where inRay hit the grid-plane in WORLD coords:
      vec4.scaleAndAdd(myHit.hitPt, inRay.orig, inRay.dir, myHit.t0);
...
CGeom.prototype.traceCone = function(inRay, myHit){
  ...
  vec4.transformMat4(rayT.orig, inRay.orig, this.worldRay2model);
  vec4.transformMat4(rayT.dir,  inRay.dir,  this.worldRay2model);
  var tz = (-1-rayT.orig[2])/rayT.dir[2];
  var tzxy = Math.pow(rayT.orig[0] + rayT.dir[0] * tz, 2) + Math.pow(rayT.orig[1] + rayT.dir[1] * tz, 2)
  var nor = vec4.create();
  if(tz > 0 && tzxy < 1){
    if(rayT.dir[2] > 0){
      if(tz > myHit.t0){
      ...
      var A = Math.pow(rayT.dir[0],2) + Math.pow(rayT.dir[1],2) - 0.25 * Math.pow(rayT.dir[2],2);
      var B = 2 * (rayT.orig[0] * rayT.dir[0] + rayT.orig[1] * rayT.dir[1]) + (rayT.dir[2] - rayT.orig[2] * rayT.dir[2]) / 2;
      var C = Math.pow(rayT.orig[0],2) + Math.pow(rayT.orig[1],2) - Math.pow(rayT.orig[2]-1, 2) / 4;
      var t1 = (-B + Math.sqrt(Math.pow(B,2) - 4 * A * C)) / (2 * A);
      var t2 = (-B - Math.sqrt(Math.pow(B,2) - 4 * A * C)) / (2 * A);
      var t = Math.min(t1, t2);
      if(t > myHit.t0){
      ...
      // Compute the x,y,z,w point where inRay hit the grid-plane in WORLD coords:
      vec4.scaleAndAdd(myHit.hitPt, inRay.orig, inRay.dir, myHit.t0);
      nor = vec4.fromValues(myHit.modelHitPt[0], myHit.modelHitPt[1], (0.5 - 0.5 * myHit.modelHitPt[2]) / 2, 0);
    }
  }
  else {
    var a = vec3.fromValues(0-rayT.orig[0], 0-rayT.orig[1],0);
    ...
    var dis = Math.sqrt(Math.pow(mora,2) - Math.pow(mora * cos,2)); 
    var disT = Math.sqrt(Math.pow(rayT.orig[0],2)+Math.pow(rayT.orig[1],2)+Math.pow(rayT.orig[2],2)) * cos / morb;
    var z = rayT.orig[2] + rayT.dir[2] * disT;
    var disZ = 0.5 - z * 0.5;
    if(dis <= disZ){
      var A = Math.pow(rayT.dir[0],2) + Math.pow(rayT.dir[1],2) - 0.25 * Math.pow(rayT.dir[2],2);
      var B = 2 * (rayT.orig[0] * rayT.dir[0] + rayT.orig[1] * rayT.dir[1]) + (rayT.dir[2] - rayT.orig[2] * rayT.dir[2]) / 2;
      ...
      var t2 = (-B - Math.sqrt(Math.pow(B,2) - 4 * A * C)) / (2 * A);
      var t = Math.min(t1, t2);
      if((0.5 - 0.5 * (rayT.orig[2] + rayT.dir[2] * t)) < 0){
        return;
      }
      var newZ = rayT.orig[2] + rayT.dir[2] * t;
      if(newZ >= -1 && t < myHit.t0){
        var temp = myHit.modelHitPt;
        vec4.scaleAndAdd(temp, rayT.orig, rayT.dir, myHit.t0); 
        myHit.t0 = t;
        vec4.scaleAndAdd(myHit.modelHitPt, rayT.orig, rayT.dir, myHit.t0); 
      // Compute the x,y,z,w point where inRay hit the grid-plane in WORLD coords:
        vec4.scaleAndAdd(myHit.hitPt, inRay.orig, inRay.dir, myHit.t0);
...
CGeom.prototype.traceGlass = function (inRay, myHit){
  var rayT = new CRay();    // to create 'rayT', our local model-space ray.
  vec4.copy(rayT.orig, inRay.orig);   // memory-to-memory copy. 
  vec4.copy(rayT.dir, inRay.dir);
                            // (DON'T do this: rayT = inRay; // that sets rayT
  ...
  var B = 2 * (rayT.orig[0] * rayT.dir[0] + rayT.orig[1] * rayT.dir[1] + rayT.orig[2] * rayT.dir[2]);
  var C = Math.pow(rayT.orig[0], 2) + Math.pow(rayT.orig[1], 2) + Math.pow(rayT.orig[2], 2) - 1;
  var t1 = (-B + Math.sqrt(Math.pow(B,2) - 4 * A * C)) / (2 * A);
  var t2 = (-B - Math.sqrt(Math.pow(B,2) - 4 * A * C)) / (2 * A);
  var t = Math.max(t1, t2);
  
  myHit.t0 = t;          // record ray-length, and
  myHit.hitGeom = this;      // record this CGeom object as the one we hit, and
  // Compute the x,y,z,w point where rayT hit the sphere in MODEL coords:
                  // vec4.scaleAndAdd(out,a,b,scalar) sets out = a + b*scalar
  vec4.scaleAndAdd(myHit.modelHitPt, rayT.orig, rayT.dir, myHit.t0); 
