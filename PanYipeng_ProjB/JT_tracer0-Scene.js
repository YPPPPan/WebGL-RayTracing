...
//  -- 'iNearest' index selects the CHit object nearest the ray's origin point.
	...
	//
  this.name = new Array();
  this.hitPt = new Array();
  this.hitNum = new Array();
}

CHitList.prototype.add = function(element, point){
  this.name.push(element);
  this.hitPt.push(point);
}

CHitList.prototype.addNum = function(element){
  this.hitNum.push(element);
}
...
CScene.prototype.setAttr = function(jitter, AAcode,light,reflect) {
  this.jitter = jitter;
  this.AAcode = AAcode;
  this.lightSwitch = light;
  this.reflect = reflect;
}

CScene.prototype.setScene = function(s) {
  this.Scene = s;
}
...
//     == 3: ground-plane grid + sphere + 3rd shape, etc.
  ...
  this.rayCam.setLight(gui.lightPt);
...
//      console.log('iNow should be 1; it is:', iNow);
      // set up distinctive coloring:
  	  vec4.set(this.item[iNow].gapColorA,  0.1,0.2,0.3,1.0); // RGBA(A==opacity) bluish gray   
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.1,0.1,1.0);  // muddy red
  	  vec4.set(this.item[iNow].gapColorD,  0.3,0.5,0.7,1.0); // RGBA(A==opacity) bluish gray   
  	  vec4.set(this.item[iNow].lineColorD, 0.7,0.3,0.3,1.0);  // muddy red
      ...
      iNow = this.item.length -1;                 // get its array index.
      // set up distinctive coloring:
      vec4.set(this.item[iNow].gapColorA,  0.0,0.0,0.3,1.0); // RGBA(A==opacity) blue
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.3,0.0,1.0);  // yellow
      vec4.set(this.item[iNow].gapColorD,  0.0,0.0,1.0,1.0); // RGBA(A==opacity) blue
  	  vec4.set(this.item[iNow].lineColorD, 1.0,1.0,0.0,1.0);  // yellow
  	  // Now apply transforms to set disk's size, orientation, & position.
  	  // (Be sure to do these same transforms in WebGL preview; find them in the
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
      ...
      iNow = this.item.length -1;                 // get its array index.
      // set up distinctive coloring:
      vec4.set(this.item[iNow].gapColorA,  0.0,0.3,0.3,1.0); // RGBA(A==opacity) blue
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.5,0.0,1.0);  // yellow
      vec4.set(this.item[iNow].gapColorD,  0.0,0.7,1.0,1.0); // RGBA(A==opacity) blue
  	  vec4.set(this.item[iNow].lineColorD, 0.7,1.0,0.0,1.0);  // yellow
  	  // Now apply transforms to set disk's size, orientation, & position.
  	  // (Be sure to do these same transforms in WebGL preview; find them in the
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
  	  this.item[iNow].rayTranslate(-2.3,1,1.3);         // move drawing axes 
  	                                                // LEFT, BACK, & UP.
      this.item[iNow].rayRotate(0.3*Math.PI, 1,0,0); // rot 135 on x axis to face us
      this.item[iNow].rayRotate(Math.PI/5, 0,0,1);    // z-axis rotate 60deg.
      ...
      // move it to a more-sensible location:
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.3,0.0,1.0);  // yellow
  	  vec4.set(this.item[iNow].lineColorD, 1.0,1.0,0.0,1.0);  // yellow
  	  this.item[iNow].shiness= 5;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(2.2, 0.5, 1.2);  // move rightwards (+x),
      this.item[iNow].rayScale(0.7, 0.7, 0.7);  // move rightwards (+x),
      ...
      // Initially leave sphere at the origin. Once you see it, then
      // move it to a more-sensible location:
  	  vec4.set(this.item[iNow].lineColorA, 0.0,0.3,0.0,1.0);  // yellow
  	  vec4.set(this.item[iNow].lineColorD, 0.0,1.0,0.0,1.0);  // yellow
  	  this.item[iNow].shiness=300;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(1.7, 0.0, 1.5);  // move rightwards (+x),
      this.item[iNow].rayScale(0.5, 0.5, 0.8);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(3, 0.0, 1.5);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-3, 0.0, 1.3);  // move rightwards (+x),

      
      this.item.push(new CGeom(RT_CONE));       // Append sphere to item[] &
      iNow = this.item.length -1;                 // get its array index.
      // Initially leave sphere at the origin. Once you see it, then
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-2, -2.0, 1.2);  // move rightwards (+x),*/
      // and toward camera (-y) enough to stay clear of disks, and up by 1 to
      // make this radius==1 sphere rest on gnd-plane.
      //
      //
    ...
    case 1:
      ...
      iNow = this.item.length -1;               // get its array index.
                                                ...
                                                // no transforms needed.
      //-----Disk 1------   
      this.item.push(new CGeom(RT_BOX));         // Append 2D disk to item[] &
      iNow = this.item.length -1;                 // get its array index.
//      console.log('iNow should be 1; it is:', iNow);
      // set up distinctive coloring:
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.1,0.1,1.0);  // muddy red
  	  vec4.set(this.item[iNow].lineColorD, 0.7,0.3,0.3,1.0);  // muddy red】
      this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(2.0, -2.0, 2.3);        // move drawing axes 
                                                    // RIGHT, BACK, & UP.
      ...
      // set up distinctive coloring:
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(3.0, -1.0, 0.3);         // move drawing axes 
  	                                                  // LEFT, BACK, & UP.
      //-----Disk 3------ 
      this.item.push(new CGeom(RT_BOX));         // Append 2D disk to item[] &
      iNow = this.item.length -1;                 // get its array index.
      // set up distinctive coloring:
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-4.3, 1.0, 4.3);         // move drawing axes 
  	                                                  // LEFT, BACK, & UP.
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(2.1, -2.0, 0.5);  // move rightwards (+x),
      this.item[iNow].rayScale(0.5, 0.5, 0.5);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  ...
  	  this.item[iNow].shiness= 5;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0.5, 0.1, 0.5);  // move rightwards (+x),
      this.item[iNow].rayScale(0.7, 1.2, 1.2);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  ...
  	  this.item[iNow].shiness=300;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-1.7, 0.0, 0.7);  // move rightwards (+x),
      this.item[iNow].rayScale(0.8, 0.8, 0.8);  // move rightwards (+x),
    //
    ...
    case 2:
      ...
      iNow = this.item.length -1;               // get its array index.
                                                ...
                                                // no transforms needed.
      //-----Disk 1------   
      this.item.push(new CGeom(RT_CYLINDER));         // Append 2D disk to item[] &
      iNow = this.item.length -1;                 // get its array index.
//      console.log('iNow should be 1; it is:', iNow);
      // set up distinctive coloring:
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.1,0.1,1.0);  // muddy red
  	  vec4.set(this.item[iNow].lineColorD, 0.7,0.3,0.3,1.0);  // muddy red】
      this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(3.0, -1.0, 1.3);        // move drawing axes 
                                                    // RIGHT, BACK, & UP.
      ...
      // set up distinctive coloring:
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-1.0, 1.0, 0.5);         // move drawing axes 
      this.item[iNow].rayScale(0.5, 0.5, 0.7);  // move rightwards (+x),
  	                                                  // LEFT, BACK, & UP.
      //-----Disk 3------ 
      this.item.push(new CGeom(RT_CYLINDER));         // Append 2D disk to item[] &
      iNow = this.item.length -1;                 // get its array index.
      // set up distinctive coloring:
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-1.5, -1.5, 2.3);         // move drawing axes 
      this.item[iNow].rayScale(0.7, 0.7, 0.5);  // move rightwards (+x),
  	                                                  // LEFT, BACK, & UP.
      ...
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(1.1, -2.0, 0.9);  // move rightwards (+x),
      this.item[iNow].rayScale(0.5, 0.5, 0.5);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  ...
  	  this.item[iNow].shiness= 5;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0.5, 0.1, 1.5);  // move rightwards (+x),
      this.item[iNow].rayScale(1.2, 1.2, 1.2);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  ...
  	  this.item[iNow].shiness=300;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-1.7, 0.0, 1.7);  // move rightwards (+x),
      this.item[iNow].rayScale(0.8, 0.8, 0.8);  // move rightwards (+x),
      break;
      ...
      iNow = this.item.length -1;               // get its array index.
                                                ...
                                                // no transforms needed.
      //-----Disk 1------   
      this.item.push(new CGeom(RT_CONE));         // Append 2D disk to item[] &
      iNow = this.item.length -1;                 // get its array index.
//      console.log('iNow should be 1; it is:', iNow);
      // set up distinctive coloring:
  	  vec4.set(this.item[iNow].lineColorA, 0.3,0.1,0.1,1.0);  // muddy red
  	  vec4.set(this.item[iNow].lineColorD, 0.7,0.3,0.3,1.0);  // muddy red】
      this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(2.0, -1.0, 1.0);        // move drawing axes 
                                                    // RIGHT, BACK, & UP.
      ...
      // set up distinctive coloring:
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-2.0, -2.0, 0.5);         // move drawing axes 
      this.item[iNow].rayScale(0.5, 0.5, 0.7);  // move rightwards (+x),
  	                                                  // LEFT, BACK, & UP.
      //-----Disk 3------ 
      this.item.push(new CGeom(RT_CONE));         // Append 2D disk to item[] &
      ...
      // set up distinctive coloring:
  	  ...
  	  //  JT_VBObox-lib.js file, in VBObox0.draw() function)
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-2.5, -0.5, 1.3);         // move drawing axes 
      this.item[iNow].rayScale(0.7, 0.7, 0.5);  // move rightwards (+x),
  	                                                  // LEFT, BACK, & UP.

      //-----Sphere 1-----
      this.item.push(new CGeom(RT_GLASS));       // Append sphere to item[] &
      iNow = this.item.length -1;                 // get its array index.
      // Initially leave sphere at the origin. Once you see it, then
      // move it to a more-sensible location:
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(0.5, -2.0, 0.3);  // move rightwards (+x),
      this.item[iNow].rayScale(0.3, 0.3, 0.3);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  ...
  	  this.item[iNow].shiness= 5;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-0.7, 0.1, 2.5);  // move rightwards (+x),
      this.item[iNow].rayScale(1.1, 1.3, 1.2);  // move rightwards (+x),
      ...
      // move it to a more-sensible location:
  	  ...
  	  this.item[iNow].shiness=300;  // yellow
  	  this.item[iNow].setIdent();                   // start in world coord axes
      this.item[iNow].rayTranslate(-1.0, 0.0, 0.7);  // move rightwards (+x),
      this.item[iNow].rayScale(0.8, 0.8, 0.8);  // move rightwards (+x),
      break;
...
//	console.log("You called CScene.makeRayTracedImage!")
  ...
  var sample = this.AAcode;
  var is_jitter = this.jitter;
    this.rayCam.rayPerspective(gui.camFovy, gui.camAspect, gui.camNear);
    this.rayCam.rayLookAt(gui.camEyePt, gui.camAimPt, gui.camUpVec);
  ...
  for(j=0; j< this.imgBuf.ySiz; j++) {        // for the j-th row of pixels.
      ...
      for(e = 0; e < Math.pow(sample, 2); e++){
        newI = i - 1 / (2 * sample) + (1 / sample * (e % sample));
        newJ = j - 1 / (2 * sample) + 1 / sample * (Math.floor(e / sample));
        if(is_jitter == 1){
          newI += Math.random() / sample - (1 / (2 * sample));
          newJ += Math.random() / sample - (1 / (2 * sample));
        }
			  this.rayCam.setEyeRay(this.eyeRay,newI,newJ);  // create ray for pixel (i,j)
      // DIAGNOSTIC:------------------------------------
        ...
        for(k=0; k< this.item.length; k++) {  // for every CGeom in item[] array,
            ...
            //this.item[k].setIdent();
            this.item[k].rayTranslate(0.0,0.0,0);
...
*/
        var lightdiv = 2;
        if((this.lightSwitch[0] == 1 && this.lightSwitch[1] == 0) || (this.lightSwitch[0] == 0 && this.lightSwitch[1] == 1)){
          lightdiv = 1;
        }
        else if(this.lightSwitch[0] == 0 && this.lightSwitch[1] == 0){
          lightdiv = 0;
        }
        for(var l = 0; l < 2; l++){
          if(lightdiv != 0){
            if(this.lightSwitch[l] == 1){
              shade = this.findShade(myHit, myHitShadow, l);
              if(shade != null){
                this.imgBuf.fBuf[idx   ] += shade[0]/ Math.pow(sample, 2)/lightdiv;	
                this.imgBuf.fBuf[idx +1] += shade[1]/ Math.pow(sample, 2)/lightdiv;
                this.imgBuf.fBuf[idx +2] += shade[2]/ Math.pow(sample, 2)/lightdiv;
              }
            }
          }
          else{
            var skyColor = vec4.fromValues(0.3,1.0,1.0,1.0);
            if(myHit.hitNum == -1){
              vec4.copy(colr,skyColor);
            }
            if(myHit.hitNum==0) {  // use myGrid tracing to determine color
              vec4.copy(colr, myHit.hitGeom.gapColorA);
            }
            else if (myHit.hitNum==1) {
              vec4.copy(colr, myHit.hitGeom.lineColorA);
            }
            this.imgBuf.fBuf[idx   ] += colr[0]/ Math.pow(sample, 2);	
            this.imgBuf.fBuf[idx +1] += colr[1]/ Math.pow(sample, 2);
            this.imgBuf.fBuf[idx +2] += colr[2]/ Math.pow(sample, 2);
          }
          }
        }
        if(this.reflect != 0){
          var Rtimes = this.reflect-1;
          reflect = this.findReflect(myHit,Rtimes);
          this.imgBuf.fBuf[idx   ] += reflect[0]/(Rtimes+1)/8;
	  	    this.imgBuf.fBuf[idx +1] += reflect[1]/(Rtimes+1)/8;
	  	    this.imgBuf.fBuf[idx +2] += reflect[2]/(Rtimes+1)/8;
        }
      }
  	}
...
CScene.prototype.findShade = function(myHit, myHitShadow, lightNo){
    ...
    if(myHit.hitNum == -1){
      return skyColor;
            ...
            var L = vec4.create();
            vec4.subtract(L, this.rayCam.lightPt[lightNo], myHit.hitPt);
            vec4.normalize(L,L);
            var N = myHit.surfNorm;
            if(myHit.hitGeom.shapeType == 6){   
              var V = vec4.create();
              vec4.subtract(V, V, this.eyeRay.dir);
              var ratio = 1/1.66;
              var k = 1 - Math.pow(ratio, 2) * (1 - Math.pow(vec4.dot(N, V), 2));
              var k2 = ratio * vec4.dot(N, V) - k;
              var T = vec4.create();
              var N2 = vec4.fromValues(N[0]*k2, N[1]*k2, N[2]*k2, 0);
              var L2 = vec4.fromValues(V[0]*ratio, V[1]*ratio, V[2]*ratio, 0);
              vec4.subtract(T, N2, L2);
              var transRay = new CRay();
              this.rayCam.setReflectRay(transRay, myHit.hitPt, T);
              var transHit = new CHit();
              transHit.init();
              myHit.hitGeom.traceTrans(transRay, transHit);
              var ratio2 = 1.66;
              var L3 = vec4.create();
              vec4.subtract(L3, L3, T);
              vec4.normalize(L3,L3);
              var N3 = vec4.create()
              N3 = vec4.subtract(N3,N3,transHit.surfNorm);
              var k3 = 1 - Math.pow(ratio2, 2) * (1 - Math.pow(vec4.dot(N3, L3), 2));
              var k4 = ratio2 * vec4.dot(N3, L3) - k3;
              var T2 = vec4.create();
              var N4 = vec4.fromValues(N3[0]*k4, N3[1]*k4, N3[2]*k4, 0);
              var L4 = vec4.fromValues(L3[0]*ratio2, L3[1]*ratio2, L3[2]*ratio2, 0);
              vec4.subtract(T2, N4, L4);
              var transRay2 = new CRay();
              this.rayCam.setReflectRay(transRay2, transHit.hitPt, T2);
              var transHit2 = new CHit();
              transHit2.init();
              for(k=0; k< this.item.length; k++) {  // for every CGeom in item[] array,
                if(this.item[k] == myHit.hitGeom){
                  continue;
                }
                this.item[k].traceMe(transRay2, transHit2);  // trace eyeRay thru it,
                //this.item[k].setIdent();
                this.item[k].rayTranslate(0.0,0.0,0);
              }
              var transShadeHit = new CHit();
              var transShade = this.findShade(transHit2,transShadeHit,1);
              return transShade;
            }
            this.rayCam.setShadowRay(this.shadowRay, myHit.hitPt, lightNo);
            myHitShadow.init();
            for(k=0; k< this.item.length; k++) {  // for every CGeom in item[] array,
                this.item[k].traceMe(this.shadowRay, myHitShadow);  // trace eyeRay thru it,
                //this.item[k].setIdent();
                this.item[k].rayTranslate(0.0,0.0,0);
            }
            var lambertian = Math.max(vec4.dot(N, L), 0.0);
            var ambition = vec4.create();
            var diffusion = vec4.create();
            if(myHit.hitNum==0) {  // use myGrid tracing to determine color
				        vec4.copy(diffusion, myHit.hitGeom.gapColorD);
				        vec4.copy(ambition, myHit.hitGeom.gapColorA);
			      }
            else if (myHit.hitNum==1) {
                vec4.copy(diffusion, myHit.hitGeom.lineColorD);
				        vec4.copy(ambition, myHit.hitGeom.lineColorA);
            }
            if(myHitShadow.hitList.name.length >= 1 && myHit.hitGeom != myHitShadow.hitList.name[myHitShadow.hitList.name.length-1]) {
                return vec4.fromValues(ambition[0],ambition[1],ambition[2],1.0);
            }
                if(lambertian > 0){
                var V = vec4.create();
                var temp = 0.0;
                ...
                vec4.subtract(V, V, this.eyeRay.dir);
                temp = vec4.dot(N,L) * 2;
                C = vec4.fromValues(N[0] * temp, N[1] * temp, N[2] * temp, N[3] * temp);
                vec4.subtract(R, C, L);
                var specularAngle = Math.max(0.0, vec4.dot(V,R));
                var specularCoe = Math.pow(specularAngle, myHit.hitGeom.shiness);
                var specular = vec4.fromValues(1.0,1.0,1.0,1.0);

                
                return vec4.fromValues(diffusion[0]*lambertian + specular[0] * specularCoe + ambition[0], diffusion[1]*lambertian + specular[1] * specularCoe + ambition[1], diffusion[2]*lambertian + specular[2] * specularCoe + ambition[2], 1.0);
              }
              else{
                return vec4.fromValues(ambition[0],ambition[1],ambition[2],1.0);
              }
...
CScene.prototype.findReflect = function(myHit, times){
    var V = vec4.create();
    var N = myHit.surfNorm;
    vec4.subtract(V, V, this.eyeRay.dir);
    vec3.normalize(V,V);
    ...
    var RR = vec4.create();
    var tempR = 0.0;
    tempR = vec4.dot(N,V) * 2;
    CR = vec4.fromValues(N[0] * tempR, N[1] * tempR, N[2] * tempR, 0);
    vec4.subtract(RR, CR, V);
    //vec4.normalize(RR,RR);
    var reflectRay = new CRay();
    var myHitReflect = new CHit();
    this.rayCam.setReflectRay(reflectRay, myHit.hitPt, RR);
    myHitReflect.init();
    for(k=0; k< this.item.length; k++) {  // for every CGeom in item[] array,
        this.item[k].traceMe(reflectRay, myHitReflect);  // trace eyeRay thru it,
        //this.item[k].setIdent();
        this.item[k].rayTranslate(0.0,0.0,0);
    }
    var myHitReflectShade = new CHit();
    var shade = this.findShade(myHitReflect,myHitReflectShade,1);
    var shade2 = vec4.create();
    if(times > 0){
        times -= 1;
        shade2 = this.findReflect(myHitReflect, times);
    }
    if(shade == null){
      return vec4.create();
    }
    vec4.add(shade,shade,shade2);
    return shade
}
