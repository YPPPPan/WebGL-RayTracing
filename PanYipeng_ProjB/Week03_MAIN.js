...
var light = [1,1];
var reflectRecursive = 0;
var lightPt = vec4.fromValues(0,0,10,1);
...
//  on-screen buttons and controls (e.g. 't' or 'T' keys; mouse-drag;...)
  ...
  //------------------------------------------
	...
	preView.switchToMe(light,this.lightPt);  // Set WebGL to render from this VBObox.
...
// advance to the next antialiasing mode.
  ...
  gui.setAttr(g_isJitter,g_AAcode,light,reflectRecursive);
  // report it:
  if(g_AAcode==1) {
    if(g_isJitter==0) {
...
function switchReflect(){
  if(reflectRecursive == 4){
    reflectRecursive = 0;
  }
  else{
    reflectRecursive++;
  ...
  gui.setAttr(g_isJitter,g_AAcode,light,reflectRecursive);
  document.getElementById('ReflectLevel').innerHTML = 
  		"Current reflect level: " + reflectRecursive +".";
}
